...
# WLED Multi-Channel Controller Build Script
...
# Project info
PROJECT_NAME="Multi-Channel WLED Controller"
VERSION=$(date +%Y%m%d)
...
Commands:
    build [ENV]         Build specific environment (default: multi_4ch)
...
Environments:
    multi_4ch          4-channel basic (300 LEDs/ch)
    multi_4ch_150led   4-channel smaller (150 LEDs/ch)
    multi_6ch          6-channel (200 LEDs/ch)
    multi_8ch          8-channel (150 LEDs/ch)
    multi_4ch_ws2815   4-channel with WS2815 (12V, 500 LEDs/ch)
    multi_commercial   6-channel commercial (mixed LED counts)
    multi_architectural 8-channel architectural (large scale)
    multi_debug        Debug build
    multi_ota          OTA update build
    multi_s3           ESP32-S3 with PSRAM (400 LEDs/ch)
    multi_test         Test build (10 LEDs/ch)

Examples:
    ./build.sh build                    # Build 4-channel configuration
    ./build.sh build multi_8ch          # Build 8-channel version
...
case "${1}" in
    ...
    *)
        if [ -z "${1}" ]; then
            ...
            echo "1) Build 4-channel configuration"
            ...
            case ${choice} in
                1) build_env multi_4ch ;;
                2) build_all ;;
                3)
                    echo "Available environments:"
                    echo "  multi_4ch, multi_6ch, multi_8ch, multi_commercial, etc."
                    read -p "Enter environment [multi_4ch]: " env
                    env=${env:-multi_4ch}
